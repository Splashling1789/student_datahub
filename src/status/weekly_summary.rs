...
pub(super) fn weekly_summary(total_time_studied : i32, times : &Vec<(Subject, i32)>, last_week: Option<i32>, average : Option<f32>) {
        ...
        if let Some(last_week) = last_week {
            ...
            if last_week != 0 {
                match total_time_studied as f32/(last_week as f32) {
                    k @ 0.0..1.0 => {
                        println!("\t - You've studied {:.1}% less than last week.", (1.0-k) * 100.0);
                    }
                    1.0 => {
                        println!("\t - You've studied just the same as last week.");
                    }
                    k @ 1.0..=f32::MAX => {
                        println!("\t - You've studied {:.1}% more than last week.", (k - 1.0) * 100.0);
                    }
                    k => {
                ...
                println!("\t - Definitely better than last week. (Last week you didn't studied at all)");
            }
        }
